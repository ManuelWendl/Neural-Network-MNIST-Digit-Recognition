...
> - Nmist Digiit Recognition Advanced Neural Network:
    This version contains a network with biases for each neuron. Additionally it is possible to fully custamize the structure of the neural network. However due to numerical error caused by expensive calculations, no varanty can be given at extreme architectures. (Optimal range 1-4 hidden layers)
